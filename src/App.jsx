...
const BRAND = {
  email: 'contact@behnjproductions.ca',
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {!sent ? <>
          ...
          <p>Cette démonstration confirme le parcours. Lors de la mise en ligne, le formulaire sera relié à votre adresse professionnelle.</p>
    ...
    <div className="site-shell">
      ...
      <main>
        <section id="accueil" className="hero">
          <img className="hero__image" src="/assets/hero-triptych.png" alt="Mariage, portrait corporatif et spectacle captés par Behn J. Productions" />
          <div className="hero__shade" />
          <div className="hero__content"><p className="eyebrow">Sept-Îles · Côte-Nord</p><h1>Des images<br />qui se vivent<span>.</span></h1><p className="hero__lead">Photographie <i /> Vidéo <i /> Diffusion web</p><div className="hero__actions"><CTAButton onClick={() => setContactOpen(true)}>Commencer mon projet</CTAButton><button className="showreel" type="button" onClick={() => scrollToSection('video')}><Play size={20} weight="fill" /> Voir la bande-démo</button></div></div>
        ...
        <section id="photographie" className="chapter chapter--photo">
          ...
          <div className="photo-triptych"><img src="/assets/photography-triptych.png" alt="Photographie corporative, mariage et événementielle" />{['Corporatif', 'Mariages', 'Événements'].map((label) => <button key={label} type="button" onClick={() => setContactOpen(true)}><span>{label}</span><ArrowRight /></button>)}</div>
        </section>

        <section id="video" className="chapter chapter--media">
          <img src="/assets/video-coast.png" alt="Production vidéo professionnelle sur la Côte-Nord" /><div className="media-shade" />
          <div className="chapter__number"><strong>02</strong><span>Des histoires<br />qui font bouger.</span></div>
          <div className="media-copy"><p className="eyebrow">Production · Réalisation · Drone</p><h2>Vidéo</h2><p>Plus que des vidéos.<br />Des émotions en mouvement.</p><CTAButton secondary onClick={() => setContactOpen(true)}>Voir nos réalisations</CTAButton></div>
        </section>

        <section id="diffusion" className="chapter chapter--media chapter--diffusion">
          <img src="/assets/diffusion-studio.png" alt="Montage et diffusion de contenu numérique" /><div className="media-shade" />
      ...
      <footer className="footer"><img src="/assets/behn-j-logo-transparent.png" alt="Behn J. Productions" /><div><strong>Sept-Îles · Québec</strong><a href={`mailto:${BRAND.email}`}>{BRAND.email}</a></div><div className="footer-links"><button onClick={() => scrollToSection('photographie')}>Photographie</button><button onClick={() => scrollToSection('video')}>Vidéo</button><button onClick={() => scrollToSection('ecoles')}>Écoles</button></div><div className="socials" aria-label="Réseaux sociaux"><a href="#instagram" aria-label="Instagram"><InstagramLogo /></a><a href="#facebook" aria-label="Facebook"><FacebookLogo /></a><a href="#youtube" aria-label="YouTube"><YoutubeLogo /></a></div></footer>
